...
![image-20200728202603151](./image/image-20200728202603151.png)
...
（4）NBIOT应用：传感器类如烟感探头、路边停车、三表（水表，电表，煤气表）等，控制类如智慧路灯。

（5）NBIOT现状：2018年开始发展，目前中国三大运营商均已建网，产品落地发展中。

（6）2G会逐步退网，部分被NBIOT替代，部分被4G替代。

（7）华为直接提供：NBIOT芯片。有研发制造NBIOT芯片的能力。
...
（2）内存管理（计算机系统的**2大核心资源**：CPU算力资源、内存资源）
...
（1）RTOS的定义和特征：实时性强，响应速度快、不复杂、不庞大、任务数不多。

（2）典型RTOS之ucos：单片机全五季课程的第4季，例如：手持打印机， 工业机器人，科研和工业生产使用比较多；

（3）典型RTOS之Vxworks军工领域用得非常多，非常专业的RTOS，也比较贵，rtlinux；
...
​		RTOS所有的任务都是运行在实地址中的，不能动态的添加任务。如果需要添加任务需要重新编译链接生成一个可执行程序(镜像文件)烧录运行。Linux 和Windows有MMUC和虚拟地址映射，所以支持我们动态的添加任务、进程，动态安装软件，比如我们下载了个QQ安装好就能用了，不用关机也不用重新编译链接。
...
（1）IoTOS的硬件平台主要是单片机， ARM contex -m, 如STM32；
...
（2）RTOS和IoTOS最大的差异在于：比起实时性参数IoTOS更在意周边生态
...
​			上海的小型创业公司, 能够不断的壮大, 可见他是有点技术的。他没有云服务。

（3）亚马逊freeRTOS

​			免费的。亚马逊除了做电商还做云计算，亚马逊开创了云计算这个行业。亚马逊收购了freeRTOS。
...
（2）基于IoTStudio和iotlink SDK的开发 （未来的开发方式）。
...
​		名字 IOT Studio已经更名为IoT  Link，所以在vscode商店下载IoT Link。

（3）vscode常用技巧请参考其他教程

（4）注意：iotstudio是IDE、是容器和工具集，而iotlink是SDK、是源码包。2者是分离合作

​			现在otstudio和iotlink已经合并了。

（5）.vscode目录下是vscode自身的文件和插件，.iotlink目录下是SDK的文件

​		.vscode路径C:\Users\administer\.vscode\extensions

​		.iotlink路径C:\Users\administer\.iotlink
...
（3）OSAL（os abstract layer）详细介绍，类似于ARM CMSIS，优势是解耦合, 将操作系统和外围对外接口标准化，可以将组件隔离开;
...
（2）SDK级的配置在**iotstudio**中，kernel级别的配置在**OS_CONFIG**中
...
（1）对于内核的四种境界：
  ...
  2. 基本读懂内核；          （10k -20k）
     ...
     看内核源码，大概知道代码做了什么，自己去写写不出来。这个课程可以带你做到这个层次； 会自己修一些小毛病；

  3. 深度理解内核；         （300K，year）

     自己写还是写不错来，知道代码为什么这么写，为什么用这个数据类型，为什么程序运行崩溃了。把车的发动机拆下来自己捣鼓捣鼓，再装回去。

  4. 自己写内核；            （1000K，year）

     我自己能设计一辆车，我自己能设计一个发动机，甚至一个整体。
...
​			systick，OS的节拍;litoes 默认配置为1ms一个节拍；OS里面的最小时间单位；

（2）OS的机器时钟：

​			cycle，CPU的最小时间单位。OS世界中最小计时单元。多个cycle构成一个systick；
